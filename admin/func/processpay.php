<?php
    ...
    require_once '../assets/php/cashier.php';

    if(isset($_POST['pagar'])){

      $comanda = $_POST['id_request'];
      $tips = $_POST['tips'];

      // var_dump($tips);
      
      $caixa = new Cashier();
      $caixa->setIdComanda($comanda);
      $cashier_id = $_SESSION['user_id'];

      $dados = $caixa->getComanda();
      
      $caixa->getTips($tips); 
      $caixa->setAllIds();

      $caixa->createCashierComanda();
      $caixa->encerraComanda();

      // var_dump($tips_val);
      // var_dump($dados->id_request);
